...
def identifica_cor_azul(frame):
	...
	cor_menor = np.array([80, 110, 100])
	cor_maior = np.array([110, 255, 255])
	#minCor, maxCor = aux.ranges('#4472c4')
	#segmentado_cor = cv2.inRange(frame_hsv, minCor, maxCor)
	...
	if not maior_contorno is None :
	    cv2.drawContours(frame, [maior_contorno], -1, [210, 100, 100 ], 5)
	    maior_contorno = np.reshape(maior_contorno, (maior_contorno.shape[0], 2))
	    media = maior_contorno.mean(axis=0)
	    media = media.astype(np.int32)
	    cv2.circle(frame, tuple(media), 5, [220, 100, 50])
	else:
	    media = (0, 0)
